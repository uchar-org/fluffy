...
class MySwipeable extends StatefulWidget {
  ...
  State<StatefulWidget> createState() {
    return _MySwipeableState();
  }
}

class _MySwipeableState extends State<MySwipeable> with TickerProviderStateMixin {
  ...
  void _handleDragUpdate(DragUpdateDetails details) {
    ...
    var newPos = _dragExtent.abs() / (context.size?.width ?? 0).toDouble();
    ...
    if (_dragExtent.abs() > movePastThresholdPixels) {
      ...
      newPos = adjustedPixelPos / (context.size?.width ?? 0).toDouble();
