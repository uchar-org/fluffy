...
class GroupCall {
  ...
  Future<void> join({
    ...
    Logs().i(
      '[CallKit.GroupCall] join: roomId=${room.id}, skipLobby=$skipLobby',
    );
    ...
    if (_connectionState != CallConnectionState.disconnected &&
      ...
      Logs().w(
        '[CallKit.GroupCall] join: cannot join, state=$_connectionState',
      );
      return;
    }
    ...
    try {
      ...
      Logs().d(
        '[CallKit.GroupCall] join: loading widget URL: ${_settings!.url}',
      );
  ...
  Future<void> hangup() async {
    ...
    if (_connectionState == CallConnectionState.disconnected) {
      Logs().w('[CallKit.GroupCall] hangup: already disconnected, skipping');
      return;
    }
  ...
  Future<String> _getLiveKitUrl() async {
    ...
    try {
      ...
      Logs().e(
        '[CallKit.GroupCall] _getLiveKitUrl: .well-known fetch failed',
        e,
        s,
      );
    }
